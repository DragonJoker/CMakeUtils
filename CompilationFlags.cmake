	...
	if ( CMAKE_COMPILER_IS_GNUCXX )
		...
		if ( MINGW )
			...
			elseif ( ${COMPILER_VERSION} LESS "49" )
				set( _C_FLAGS "${_C_FLAGS} -fPIC" )
				set( _CXX_FLAGS "${_CXX_FLAGS} -fPIC -std=gnu++11 ${SSE2_FLAG}" )
			else ()
				set( _C_FLAGS "${_C_FLAGS} -fPIC" )
				set( _CXX_FLAGS "${_CXX_FLAGS} -fPIC -std=gnu++1y ${SSE2_FLAG} -fdiagnostics-color=auto" )
			endif()
		else ()
			if ( ${COMPILER_VERSION} LESS "47" )
				set( _C_FLAGS "${_C_FLAGS} -fPIC" )
				set( _CXX_FLAGS "${_CXX_FLAGS} -fPIC -std=gnu++0x ${SSE2_FLAG}" )
			elseif ( ${COMPILER_VERSION} LESS "49" )
				set( _C_FLAGS "${_C_FLAGS} -fPIC" )
				set( _CXX_FLAGS "${_CXX_FLAGS} -fPIC -std=c++11 ${SSE2_FLAG}" )
			else ()
				set( _C_FLAGS "${_C_FLAGS} -fPIC" )
				set( _CXX_FLAGS "${_CXX_FLAGS} -fPIC -std=c++1y ${SSE2_FLAG} -fdiagnostics-color=auto" )
			endif ()
		endif ()
